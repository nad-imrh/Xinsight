...
export function SentimentAnalysisChart({ sentiment, brand = 'disney' }: SentimentAnalysisChartProps) {
  ...
  const renderExamples = (examples?: Tweet[], category?: string) => {
    if (!examples || examples.length === 0) {
      return <p className="text-xs text-slate-400 italic">Tidak ada contoh tweet</p>
    }
    
    return (
      <div className="space-y-2">
        {examples.map((tweet, idx) => (
          <div key={idx} className="text-xs p-2 rounded bg-slate-800/30 border border-slate-700">
            <p className="line-clamp-2 text-slate-200">{tweet.full_text || tweet.text}</p>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Analisis sentimen dari tweet</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <div className="space-y-4">
            <div>
              <h4 className="text-sm font-semibold mb-2 flex items-center gap-2 text-white">
                <span className="w-3 h-3 rounded-full" style={{ backgroundColor: SENTIMENT_COLORS.positive }}></span>
                Positif ({sentiment?.positive || 0}%)
              </h4>
              {renderExamples(sentiment?.positiveExamples, 'positive')}
            </div>

            <div>
              <h4 className="text-sm font-semibold mb-2 flex items-center gap-2 text-white">
                <span className="w-3 h-3 rounded-full" style={{ backgroundColor: SENTIMENT_COLORS.neutral }}></span>
                Netral ({sentiment?.neutral || 0}%)
              </h4>
              {renderExamples(sentiment?.neutralExamples, 'neutral')}
            </div>

            <div>
              <h4 className="text-sm font-semibold mb-2 flex items-center gap-2 text-white">
                <span className="w-3 h-3 rounded-full" style={{ backgroundColor: SENTIMENT_COLORS.negative }}></span>
                Negatif ({sentiment?.negative || 0}%)
              </h4>
              {renderExamples(sentiment?.negativeExamples, 'negative')}
            </div>
          </div>
        </div>
      </CardContent>
